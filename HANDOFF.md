...
| **4 — View-sync + earnings worker** | BullMQ worker (`worker/`): view-sync → snapshots → `record_clip_earning`; provider abstraction (simulated/Ayrshare); basic fraud velocity check. |
...
- **View provider abstraction** (`worker/views-provider.ts`) — `isAyrshareEnabled()` picks the real Ayrshare provider vs the simulated one; switching to real is intended to be a single env change.
...
- **Mock mode: fully working.** Builds, 119/119 unit tests pass, all flows interactive in the browser.
- **Real mode: built but unproven.** All real-mode code typechecks/builds and the SQL/worker logic is unit-tested, but **none of it has executed against a real Supabase + Redis + Stripe**. Migrations are unapplied; the worker is not deployed; no Redis is provisioned.
...
2. **Ayrshare account-linking is not implemented.** The provider, `profiles.ayrshare_profile_key`, and a disabled UI placeholder exist, but the OAuth/Profile-Key flow is missing and the real response parsing is unverified. Without a key, views are **simulated**. View data = money, so this is the biggest external unknown.
...
6. **Brand moderation queue shows creator email instead of name** in real mode (the `clips.creator_id → profiles` join was deferred).
7. **Mock/real dual code paths** everywhere — doubles surface and invites drift (the recently-fixed discover `campaign_type` bug was exactly this).

**Low / cosmetic**
8. Some legacy dashboard data is **hardcoded/fake** (e.g. follower counts, ROI figures, mock invites, mock social verification).
9. **~158 pre-existing lint errors** across legacy files.
10. No worker **monitoring/alerting**; retries are silent.
...
Architecturally solid and demo-complete in mock mode, with a well-designed, idempotent, atomic money pipeline — **but not yet proven against real infrastructure.** The single most important next action is one live end-to-end run on staging.
