...
2. **Scripted health probe** — `npm run monitor:health` fails if the app health endpoint is unreachable or degraded. This is ready for a scheduled runner once the GitHub/Vercel automation credentials are available.
...
| Scheduled runner secret | `AETHER_WORKER_HEALTH_URL` | Optional worker health URL if exposed through a private monitor/proxy. |
| Scheduled runner variable | `AETHER_APP_HEALTH_URL` | Overrides the default app health endpoint. |

## Current Limitation

The Hetzner worker health port is intentionally bound to `127.0.0.1`, so an external scheduled probe cannot check it directly. Verify worker runtime over SSH with:
