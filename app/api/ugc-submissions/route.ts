...
/** Shared concurrency budget for performance post submissions. */
const UGC_WRITE_MAX_CONCURRENCY = 40;

export const GET = () => methodNotAllowed(["POST"]);

export async function POST(request: Request): Promise<Response> {
  const slot = getLimiter("ugc-write", UGC_WRITE_MAX_CONCURRENCY).tryAcquire();
  if (!slot) return busyResponse();
  try {
    return await handleUgcSubmit(request);
  } finally {
    slot.release();
  }
}

async function handleUgcSubmit(request: Request): Promise<Response> {
  const guarded = await guardApiPost(request, {
    schema: ClipSubmitBodySchema,
    rateLimit: "submit",
    routeKey: "ugc-submissions/submit",
    auth: "influencer",
  });
