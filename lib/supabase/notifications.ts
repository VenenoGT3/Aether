...
export async function getNotifications(userId: string): Promise<{ data: NotificationItem[]; error: unknown }> {
  if (isMockMode) {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem(STORAGE_KEY);
      const notifications = stored ? JSON.parse(stored) : [];
      const userNotifs = notifications.filter((n: NotificationItem) => n.user_id === userId);
...
export async function markNotificationRead(notificationId: string): Promise<{ success: boolean; error: unknown }> {
  if (isMockMode) {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem(STORAGE_KEY);
      if (stored) {
        const notifications = JSON.parse(stored);
        const updated = notifications.map((n: NotificationItem) => 
...
export async function markAllNotificationsRead(userId: string): Promise<{ success: boolean; error: unknown }> {
  if (isMockMode) {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem(STORAGE_KEY);
      if (stored) {
...
export async function deleteNotification(notificationId: string): Promise<{ success: boolean; error: unknown }> {
  if (isMockMode) {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem(STORAGE_KEY);
      if (stored) {
        const notifications = JSON.parse(stored);
        const filtered = notifications.filter((n: NotificationItem) => n.id !== notificationId);
