...
// CTA links must point at the deployed origin, not a hardcoded domain.
function appUrl(path: string): string {
  const base = process.env.NEXT_PUBLIC_APP_URL?.trim().replace(/\/$/, "") || "https://aether.co";
  return `${base}${path}`;
}
...
export async function sendPaymentReleasedEmail(toEmail: string, brandName: string, campaignTitle: string, payout: number) {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; color: #1d1d1f; line-height: 1.5;">
      ...
      <a href="${appUrl("/creator/dashboard")}" style="display: inline-block; background-color: #007aff; color: #ffffff; text-decoration: none; font-size: 14px; font-weight: 600; padding: 12px 30px; border-radius: 24px;">View Wallet Balance</a>
...
export async function sendNewMessageEmail(toEmail: string, senderName: string, messageContent: string, campaignTitle: string) {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; color: #1d1d1f; line-height: 1.5;">
      ...
      <a href="${appUrl("/campaigns")}" style="display: inline-block; background-color: #007aff; color: #ffffff; text-decoration: none; font-size: 14px; font-weight: 600; padding: 12px 30px; border-radius: 24px;">Open Direct Chat</a>
