 ...
 * Stripe amounts are major currency units in our API surface). Comparing or summing those
 * floats directly needs ±0.01 tolerance hacks; converting to integer cents at
 * the boundary makes equality and ordering exact.
 */

/** Major currency units → integer cents. 19.999999999999998 → 2000. */
export function toCents(amount: number): number {
  if (!Number.isFinite(amount)) return NaN;
  return Math.round(amount * 100);
}

/** Integer cents → major currency units (exact: cents/100 is representable enough for 2dp). */
...
/** Sum money amounts without accumulating float drift (adds in cents). */
